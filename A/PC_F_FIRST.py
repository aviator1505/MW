...
thisExp = data.ExperimentHandler(name='MW_PC_Eyelink', version='1.0',  # not needed, just handy
...
pc_img = visual.ImageStim(win, image='PC_v2.jpg')
############################### Stimuli ###############################
# Screen 1
...
for idx, val in instrFile.iteritems():
    ...
    img = visual.ImageStim(win, val, size=(1920, 1080))
    img.draw()
    win.flip()
...
#            pass
    ...
    page = visual.ImageStim(win, image=stimFile.iloc[current_index], size=(1920,1080))
    page.draw()
    win.flip()
